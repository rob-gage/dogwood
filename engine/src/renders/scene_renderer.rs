        ...
        let bind_group: Option<wgpu::BindGroup> = scene.map(|scene| {
            ...
            accelerator.wgpu_device().create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    ...
                    wgpu::BindGroupEntry {
                        binding: 6,
                        resource: graphics.fluid_material_identifiers
                            .wgpu_buffer().as_entire_binding(),
                    },
                    wgpu::BindGroupEntry {
                        binding: 7,
...
mod tests {
    ...
    #[test]
    fn paused_scene_edit_renders_fluid_without_canonical_material() {
        ...
        let graphics = scene.graphics();
        assert!(read_u32(&accelerator, graphics.cellular_material_identifiers, 64)
            .into_iter().all(|material| material == 0));
        assert!(read_u32(&accelerator, graphics.fluid_material_identifiers, 64)[0]
            == water.as_u32());
        let texture: wgpu::Texture = accelerator.wgpu_device().create_texture(
            &wgpu::TextureDescriptor {
                label: Some("scene renderer test target"),
        ...
        let target: wgpu::TextureView = texture.create_view(&Default::default());
        let mut encoder: wgpu::CommandEncoder = accelerator.wgpu_device().create_command_encoder(
            &wgpu::CommandEncoderDescriptor { label: Some("scene renderer test") },
        );
        ...
        let staging: wgpu::Buffer = accelerator.wgpu_device().create_buffer(
            &wgpu::BufferDescriptor {
                label: Some("scene renderer test readback"),
                size: 256,
        ...
        let pixel = staging.slice(..).get_mapped_range().unwrap();
        assert!(pixel[0] > 0 && pixel[1] > pixel[0] && pixel[2] > pixel[1]);
        drop(pixel);
        staging.unmap();
    }
    ...
    fn read_u32(
        accelerator: &Accelerator,
        source: &engine_compute::AcceleratorBuffer,
        count: usize,
    ) -> Vec<u32> {
        let staging: wgpu::Buffer = accelerator.wgpu_device().create_buffer(
            &wgpu::BufferDescriptor {
                label: Some("scene buffer test readback"),
                size: (count * 4) as u64,
                usage: wgpu::BufferUsages::MAP_READ | wgpu::BufferUsages::COPY_DST,
                mapped_at_creation: false,
            },
        ...
        let mut encoder: wgpu::CommandEncoder = accelerator.wgpu_device().create_command_encoder(
            &wgpu::CommandEncoderDescriptor { label: Some("scene buffer test readback") },
        );
