...
impl<G: Game> GameApplication<G> {
    ...
    fn render_window(&mut self) {
        let frame = match self.render_context.as_ref().map(|context| context.surface().get_current_texture()) {
            None => return,
            ...
            | Some(wgpu::CurrentSurfaceTexture::Lost) => {
                let (Some(accelerator), Some(render_context)) =
                    (self.accelerator.as_ref(), self.render_context.as_ref()) else { return; };
    ...
    fn resumed(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
        ...
        match event_loop.create_window(attributes) {
            Ok(window) => {
                ...
                let accelerator = match pollster::block_on(Accelerator::new(instance, &surface)) {
