fn is_hard_external_body(occupancy: u32) -> bool {
    return
    occupancy == HARD_EXTERNAL_BODY_OCCUPANCY || occupancy == RIGID_EXTERNAL_BODY_OCCUPANCY;
}

// Removes every authoritative particle whose current world cell was edited
@compute @workgroup_size(64)
fn remove_edited_fluid_particles(
    ...
    if cell_index == INVALID_PHYSICAL_CELL_INDEX || edit_cells[cell_index] == EMPTY_MATERIAL_IDENTIFIER {
        return;
    }
...
@compute @workgroup_size(64)
fn spawn_edited_fluid_particles(
    @builtin(global_invocation_id) invocation: vec3<u32>
) {
    ...
    if material_form_from_identifier(
      material_identifier) != FLUID_MATERIAL_FORM || cellular_material_identifiers[cell_index] != EMPTY_MATERIAL_IDENTIFIER {
        return;
    }
...
@compute @workgroup_size(64)
fn classify_active_fluid_particles(
    @builtin(global_invocation_id) invocation: vec3<u32>
) {
    let particle_index: u32 = invocation.x;
    if particle_index >= parameters.particle_capacity || particles[particle_index].material_identifier == EMPTY_MATERIAL_IDENTIFIER {
    ...
    particles[particle_index].is_active = select(
      0u,
      1u,
      fluid_position_is_inside_active_region(
...
fn predict_fluid_particles(
    ...
    let maximum_speed: f32 = f32(
      parameters.maximum_movement_cells) / (CELLS_PER_TILE_FLOAT * parameters.delta_time);
    ...
    let step_count: u32 = clamp(
      u32(ceil(movement_cells * 2.0)),
      1u,
...
fn insert_committed_fluid_particles_into_spatial_buckets(
    ...
    let bucket_index: u32 = fluid_spatial_bucket_index_from_position(
      particles[particle_index].position);
...
fn insert_predicted_fluid_particles_into_spatial_buckets(
    ...
    if !fluid_position_supports_active_neighbor_buckets(
      particles[particle_index].position) {
        return;
    }
    let bucket_index: u32 = fluid_spatial_bucket_index_from_position(
...
@compute @workgroup_size(64)
fn apply_fluid_position_corrections(
    @builtin(global_invocation_id) invocation: vec3<u32>
) {
    let particle_index: u32 = invocation.x;
    if particle_index >= parameters.particle_capacity || particles[particle_index].material_identifier == EMPTY_MATERIAL_IDENTIFIER {
    ...
    let corrected: vec2<f32> = predicted_positions[particle_index] + position_corrections[particle_index];
...
fn commit_fluid_particles(
    ...
    let substep_delta_time: f32 = parameters.delta_time / PBF_SUBSTEP_COUNT;
    var velocity: vec2<f32> = (position + particles[particle_index].prediction_collision_displacement - particles[particle_index].position) / substep_delta_time;
    let maximum_speed: f32 = f32(
      parameters.maximum_movement_cells) / (CELLS_PER_TILE_FLOAT * parameters.delta_time);
...
fn calculate_fluid_velocity_smoothing(
    ...
    position_corrections[particle_index] = calculate_fluid_particle_velocity_smoothing(particle_index, particle);
}
...
fn apply_fluid_cellular_contact_velocity_to_particles(
    ...
    let cell: vec2<i32> = vec2<i32>(floor(particles[particle_index].position * CELLS_PER_TILE_FLOAT,));
    let index: u32 = fluid_physical_cell_index_from_world_cell(cell);
    if index == INVALID_PHYSICAL_CELL_INDEX || derived_material_identifiers[index] != particles[particle_index].material_identifier {
        return;
    }
