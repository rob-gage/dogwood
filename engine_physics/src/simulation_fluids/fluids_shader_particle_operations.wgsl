...
fn predict_fluid_particles(
  ...
  for (
    var movement_step: u32 = 0u;
    movement_step < step_count;
    movement_step++
  ) {
    position += particle.velocity * substep_delta_time / f32(step_count);
    let resolved: vec2<f32> =
      project_fluid_particle_out_of_cellular_collision(position);
...
fn insert_committed_fluid_particles_into_spatial_buckets(
  ...
  let bucket_index: u32 =
    fluid_spatial_bucket_index_from_position(
      particles[particle_index].position);
  if bucket_index == INVALID_FLUID_BUCKET_INDEX {
    return;
  }
  next_particle[particle_index] =
...
fn insert_predicted_fluid_particles_into_spatial_buckets(
  ...
  if
    !fluid_position_supports_active_neighbor_buckets(
      particles[particle_index].position)
  {
    return;
  }
...
fn calculate_fluid_position_corrections(
  ...
  var correction_cells: vec2<f32> =
    calculate_fluid_particle_position_correction_cells(
      particle_index,
      position,);
  ...
  position_corrections[particle_index] =
    correction_cells / CELLS_PER_TILE_FLOAT;
}
...
fn resolve_fluid_cellular_contact_velocity(
  ...
  let velocity: vec2<f32> =
    apply_fluid_swimmer_velocity_entrainment(
      index,
      initial_velocity,
      material_identifier,);
  derived_velocity[index] =
    vec4<f32>(initial_velocity, velocity - initial_velocity);
}
...
fn apply_fluid_swimmer_velocity_entrainment(
  ...
  material_identifier: u32,) -> vec2<f32> {
  if
    external_body_occupancy[physical_cell_index] != SWIMMER_EXTERNAL_BODY_OCCUPANCY
  {
    return fluid_velocity;
  }
  let viscosity: f32 =
    max(0.0, fluid_physical_properties_from_identifier(material_identifier).y,);
  let coupling: f32 =
    clamp(
      1.0 - exp(
        -viscosity
          * derived_coverage[physical_cell_index]
...
fn apply_fluid_cellular_contact_velocity_to_particles(
  ...
  let index: u32 = fluid_physical_cell_index_from_world_cell(cell);
  if
    index == INVALID_PHYSICAL_CELL_INDEX || derived_material_identifiers[index] != particles[particle_index].material_identifier
  {
    return;
  }
