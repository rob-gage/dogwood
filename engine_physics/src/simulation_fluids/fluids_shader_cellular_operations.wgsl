fn export_fluid_particles(
    ...
    if particle_index >= parameters.particle_capacity || particles[particle_index].material_identifier == EMPTY_MATERIAL_IDENTIFIER {
        return;
    }
    let tile: vec2<i32> = vec2<i32>(floor(particles[particle_index].position));
    let relative: vec2<i32> = tile - parameters.streaming_origin;
    if any(relative < vec2<i32>(0)) || relative.x >= i32(parameters.streaming_tile_size.x) || relative.y >= i32(parameters.streaming_tile_size.y) {
        return;
    }
...
fn scatter_fluid_mechanical_response(
    ...
    let particle_index: u32 = invocation.x;
    if particle_index >= parameters.particle_capacity || particles[particle_index].material_identifier == EMPTY_MATERIAL_IDENTIFIER || particles[particle_index].is_active == 0u {
        return;
    }
    let cell: vec2<i32> = vec2<i32>(floor(particles[particle_index].position * CELLS_PER_TILE_FLOAT,));
    ...
    for (var bucket_y: i32 = -1; bucket_y <= 1; bucket_y++) {
        for (var bucket_x: i32 = -1; bucket_x <= 1; bucket_x++) {
          ...
          base_bucket_coordinates + vec2<i32>(bucket_x, bucket_y),);
            ...
            for (var chain_length: u32 = 0u; particle_index != INVALID_FLUID_PARTICLE_INDEX && chain_length < parameters.particle_capacity; chain_length++
      ) {
                let particle: Particle = particles[particle_index];
                if particle.material_identifier != EMPTY_MATERIAL_IDENTIFIER && particle.is_active != 0u && fluid_particle_belongs_to_cell(
              particle.position,
...
fn sample_fluid_state_inside_pawn_capsule(
    ...
    let shape: ActorShape = actor_shape_from_parameters(
    ...
    let minimum: vec2<i32> = vec2<i32>(
      floor((parameters.sample_center - extent) * CELLS_PER_TILE_FLOAT,));
    let maximum: vec2<i32> = vec2<i32>(
      floor((parameters.sample_center + extent) * CELLS_PER_TILE_FLOAT,));
    let sample: DerivedFluidActorSample = gather_derived_fluid_sample_inside_actor(shape, minimum, maximum,);
...
fn gather_derived_fluid_sample_inside_actor(
    ...
    for (var y: i32 = minimum.y; y <= maximum.y; y++) {
        for (var x: i32 = minimum.x; x <= maximum.x; x++) {
            ...
            let world_position: vec2<f32> = (vec2<f32>(cell) + vec2<f32>(0.5)) / CELLS_PER_TILE_FLOAT;
            if !world_position_is_inside_actor_shape(world_position, shape) {
                continue;
            }
            capsule_cell_count   += 1.0;
            let index: u32 = fluid_physical_cell_index_from_world_cell(cell);
            if index == INVALID_PHYSICAL_CELL_INDEX || derived_material_identifiers[index] == EMPTY_MATERIAL_IDENTIFIER {
                continue;
            }
...
fn calculate_fluid_particle_velocity_smoothing(
    ...
    let base_bucket_coordinates: vec2<i32> = fluid_spatial_bucket_coordinates_from_position(particle.position);
    ...
    for (var bucket_y: i32 = -1; bucket_y <= 1; bucket_y++) {
        for (var bucket_x: i32 = -1; bucket_x <= 1; bucket_x++) {
          ...
          base_bucket_coordinates + vec2<i32>(bucket_x, bucket_y),);
            ...
            for (var chain_length: u32 = 0u; neighbor_index != INVALID_FLUID_PARTICLE_INDEX && chain_length < parameters.particle_capacity; chain_length++
      ) {
    ...
    return
    fluid_constraint_properties_from_identifier(particle.material_identifier).z * difference_sum / max(weight_sum, 0.000001);
}
...
fn calculate_fluid_density_constraint_lambda(
    ...
    let base_bucket_coordinates: vec2<i32> = fluid_spatial_bucket_coordinates_from_position(position);
        ...
        for (var bucket_x: i32 = -1; bucket_x <= 1; bucket_x++) {
          ...
          base_bucket_coordinates + vec2<i32>(bucket_x, bucket_y),);
            ...
            for (var chain_length: u32 = 0u; neighbor_index != INVALID_FLUID_PARTICLE_INDEX && chain_length < parameters.particle_capacity; chain_length++
                ...
                if neighbor_index != particle_index {
                    let separation: vec2<f32> = (position - predicted_positions[neighbor_index]) * CELLS_PER_TILE_FLOAT;
                    let distance: f32 = length(separation);
                    if distance < parameters.support_radius_cells {
                        density   += fluid_poly6_kernel_weight(distance);
                        let gradient: vec2<f32> = fluid_spiky_kernel_gradient(separation, distance,) / rest_density;
            ...
            for (var chain_length: u32 = 0u; neighbor_index != INVALID_FLUID_PARTICLE_INDEX && chain_length < parameters.particle_capacity; chain_length++
                ...
                if neighbor_index != particle_index {
                    ...
                    if distance > 0.000001 && distance < parameters.support_radius_cells {
                        correction_cells   +=
              (lambdas[particle_index] + lambdas[neighbor_index] + calculate_fluid_artificial_pressure(
                  distance,
                  material_identifier)) * fluid_spiky_kernel_gradient(separation, distance) / rest_density;
                    }
                }
                neighbor_index = next_particle[neighbor_index];
            }
...
fn fluid_poly6_kernel_weight(distance: f32) -> f32 {
    ...
    return
    4.0 * difference * difference * difference / (PI * h * h * h * h * h * h * h * h);
}
...
> {
    ...
    return
    -30.0 * remaining * remaining / (PI * h * h * h * h * h) * separation / distance;
}
...
fn calculate_fluid_artificial_pressure(
    ...
    return
    -fluid_constraint_properties_from_identifier(material_identifier).y * squared * squared;
}
    ...
    for (var iteration: u32 = 0u; iteration < 2u; iteration++) {
        ...
        for (var offset_y: i32 = -1; offset_y <= 1 && !resolved; offset_y++) {
            for (var offset_x: i32 = -1; offset_x <= 1; offset_x++) {
                ...
                let index: u32 = fluid_physical_cell_index_from_world_cell(cell);
                if index == INVALID_PHYSICAL_CELL_INDEX || (cellular_material_identifiers[index] == EMPTY_MATERIAL_IDENTIFIER && !is_hard_external_body(
...
fn fluid_position_is_inside_buffered_region(position: vec2<f32>) -> bool {
    ...
    return
    all(relative >= vec2<f32>(0.0)) && relative.x < f32(parameters.buffered_tile_size.x) && relative.y < f32(parameters.buffered_tile_size.y);
}
...
fn fluid_position_is_inside_active_region(position: vec2<f32>) -> bool {
    ...
    return
    all(relative >= vec2<f32>(0.0)) && relative.x < f32(parameters.active_tile_size.x) && relative.y < f32(parameters.active_tile_size.y);
}
...
fn fluid_position_supports_active_neighbor_buckets(
    ...
    let predicted_movement_cells: f32 = f32(
...
fn fluid_position_is_inside_active_padding(
    ...
    let maximum: vec2<f32> = vec2<f32>(parameters.active_origin) + vec2<f32>(parameters.active_tile_size) + vec2<f32>(padding);
