...
impl SceneChunk {
    ...
    pub fn deserialize<R: io::Read>(reader: &mut R) -> Result<SceneChunk, io::Error> {
        ...
        let mut tile_data: Vec<TileInactive> = Vec::with_capacity(64);
        for _ in 0..64 { tile_data.push(TileInactive::deserialize(reader)?); }
        let mut tile_data = tile_data.into_iter();
        let tiles: [[TileInactive; 8]; 8] = array::from_fn(|_| {
    ...
    /// of bounds
    pub fn get_tile(&self, position: TileCoordinates) -> Result<&TileInactive, ()> {
    ...
    pub fn get_tile_unchecked(&self, position: TileCoordinates) -> &TileInactive {
        self.get_tile(position).unwrap()
    }

    /// Sets a provided `TileInactive` at a given position in this `SceneChunk` if it is not out of
    /// bounds
    pub fn set_tile(&mut self, position: TileCoordinates, tile: TileInactive) -> Result<(), ()> {
        let x: usize = usize::try_from(
            position.x.checked_sub(self.tile_coordinates.x).ok_or(())?
        ).map_err(|_| ())?;
        let y: usize = usize::try_from(
    ...
    /// Sets a provided `TileInactive` at a given position in this `SceneChunk` if it is not out of
    /// bounds, panicking if it is
    pub fn set_tile_unchecked(&mut self, position: TileCoordinates, tile: TileInactive) {
