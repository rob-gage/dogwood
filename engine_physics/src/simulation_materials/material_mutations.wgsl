...
@group(0) @binding(2) var<storage, read_write> cellular_material_identifiers: array<
  u32
>;
...
@group(0) @binding(6) var<storage, read_write> cellular_kinematics: array<
  vec4<f32>
>;
...
@group(0) @binding(10) var<storage, read_write> fluid_edits_pending: array<
  atomic<u32>
>;
@group(0) @binding(11) var<storage, read_write> cellular_amounts: array<f32>;
@group(0) @binding(12) var<storage, read_write> cellular_temperatures: array<
  f32
>;
@group(0) @binding(13) var<storage, read_write> fluid_edit_amounts: array<f32>;
@group(0) @binding(14) var<storage, read_write> fluid_edit_temperatures: array<
  f32
>;
...
@group(0) @binding(18) var<storage, read_write> fluid_free_count: array<
  atomic<u32>
>;
...
@group(0) @binding(19) var<storage, read_write> gas_fluid_candidates: array<
  GasFluidCandidate
>;
...
fn resolve_material_mutations_nonallocating(
  ...
  if
    (request.kind != 0u || cellular_material_identifiers[request.cell] != request.expected_source)
  {
    return;
  }
  ...
  let result_amount =
    select(source_amount, bitcast<f32>(request.amount), request.amount != 0u);
  let result_temperature =
    select(
      source_temperature,
...
fn place_cell(request: Request) -> bool {
  if
    (cellular_material_identifiers[request.cell] != EMPTY_MATERIAL_IDENTIFIER)
  {
    return false;
  }
  let form = material_form_from_identifier(request.replacement);
...
fn resolve_gas_fluid_condensation(
  ...
  let valid =
    candidate.replacement != EMPTY_MATERIAL_IDENTIFIER && candidate.amount > 0.0;
  condensation_amount[lane] = select(0.0, candidate.amount, valid);
  condensation_temperature[lane] =
    select(0.0, candidate.amount * candidate.temperature, valid);
  condensation_position[lane] =
    select(vec2<f32>(0.0), candidate.amount * candidate.position, valid);
  workgroupBarrier();
  var stride = 32u;
  loop {
    if (lane < stride) {
