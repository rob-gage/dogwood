...
@compute @workgroup_size(64) fn resolve_material_mutations_nonallocating(@builtin(global_invocation_id) invocation: vec3<u32>) {
 ...
 if (request.kind == 2u) { resolve_gas_nonallocating(request); return; }
 ...
 if (form == CELLULAR_DYNAMIC_MATERIAL_FORM) { cellular_material_identifiers[request.cell]=request.replacement;cellular_amounts[request.cell]=result_amount;cellular_temperatures[request.cell]=result_temperature; cellular_integrities[request.cell]=0.0; fluid_edits[request.cell]=FLUID_EDIT_ERASE; atomicStore(&fluid_edits_pending[0],1u); return; }
...
fn release_particle(index:u32) { if(index>=arrayLength(&particles)){return;} particles[index].material_identifier=EMPTY_MATERIAL_IDENTIFIER; particles[index].is_active=0u; var n=atomicLoad(&fluid_free_count[0]); loop { if(n>=arrayLength(&fluid_free_indices)){return;} let r=atomicCompareExchangeWeak(&fluid_free_count[0],n,n+1u); if(r.exchanged){fluid_free_indices[n]=index;return;} n=r.old_value; } }
fn claim_particle() -> u32 { var n=atomicLoad(&fluid_free_count[0]); loop { if(n==0u || n>arrayLength(&fluid_free_indices)){return 0xffffffffu;} let r=atomicCompareExchangeWeak(&fluid_free_count[0],n,n-1u); if(r.exchanged){let index=fluid_free_indices[n-1u];if(index>=arrayLength(&particles)){return 0xffffffffu;}return index;} n=r.old_value; } return 0xffffffffu; }
fn spawn_fluid(request:Request) -> bool { let index=claim_particle(); if(index==0xffffffffu){return false;} particles[index]=Particle(request.replacement,1u,vec2<f32>(bitcast<f32>(request.world_x),bitcast<f32>(request.world_y)),vec2<f32>(0.0),vec2<f32>(0.0),bitcast<f32>(request.amount),bitcast<f32>(request.temperature)); return true; }
