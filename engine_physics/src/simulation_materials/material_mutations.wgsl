...
fn prepare_material_mutation_dispatch(@builtin(global_invocation_id) invocation: vec3<u32>) {
    if (invocation.x != 0u) {
        return;
    }
...
fn resolve_material_mutations_nonallocating(@builtin(global_invocation_id) invocation: vec3<u32>) {
    let i = invocation.x;
    ...
    let result_temperature =
        select(source_temperature, bitcast<f32>(request.temperature), request.temperature != 0u);
