...
fn apply_canonical(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let remaining1 =
        select(0.0, max(source1_amount - coefficient1 * candidate.extent, 0.0), source1_canonical);
...
fn discover_canonical(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    for (
        ...
        let extent0 =
            select(1.0, source0.amount / max(bitcast<f32>(rule.words[2]), 0.000001), first_present);
        let extent1 =
            select(
                1.0,
