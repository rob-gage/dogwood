fn apply_canonical(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    if (candidate.reaction == 0xffffffffu || candidate.reaction >= arrayLength(&reactions) || candidate.extent <= 0.000001 || candidate.padding0 != 1u ){
        return;
    }
    ...
    let source1 = Source(
      ...
      candidate.material1,
      0.0,
      second_present && candidate.partner != 0xffffffffu && candidate.material1 != EMPTY_MATERIAL_IDENTIFIER,
  ...
  // Apply is deliberately a commit-only pass and never re-discovers sources.
    ...
    if (source0.rigid_claim != 0xffffffffu ){
        let demand = coefficient0 * candidate.extent + select(
        0.0,
    ...
    if (source0_canonical ){
        ...
        if (needs_mutation ){
          ...
          remaining <= 0.00001 && cellular_product == EMPTY_MATERIAL_IDENTIFIER);
            let result_amount = select(
          remaining,
          cellular_amount,
          cellular_product != EMPTY_MATERIAL_IDENTIFIER);
            mutation_requests[slot] = Request(
          cell,
          0u,
          cell,
          source0.material,
    ...
    if (source1_canonical && !same_canonical ){
        if (remaining1 <= 0.00001 ){
            let slot = candidate.padding1 + select(
          0u,
          1u,
    ...
    if (first_present && source0_form == GAS_MATERIAL_FORM ){
        let gas_index = material_index_from_identifier(
    ...
    if (second_present && source1_form == GAS_MATERIAL_FORM && !(source0_form == GAS_MATERIAL_FORM && source1.material == source0.material && source1.cell == source0.cell) ){
        let gas_index = material_index_from_identifier(
...
fn environment_matches(
    ...
    return
    temperature >= min_temperature && temperature <= max_temperature && pressure >= min_pressure && pressure <= max_pressure && air >= min_air && air <= max_air;
}
...
fn discover_canonical(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    for (var reaction_index = 0u; reaction_index < parameters.reaction_count && reaction_index < arrayLength(
      &reactions); reaction_index   += 1u
  ) {
    ...
    // the combined demand instead of counting the snapshot inventory twice.
        ...
        if (source0.found && material_form_from_identifier(source0.material) != GAS_MATERIAL_FORM && source1.found && source1.cell != cell && source0.amount <= bitcast<f32>(rule.words[2]) * extent + 0.00001 ){
            output_cell = source1.cell;
        }
        var gas_product = 0.0;
        for (var product = 0u; product < 2u; product   += 1u) {
            let base = 8u + product * 4u;
            if (rule.words[base + 2u] != 0u && material_form_from_identifier(
          rule.words[base]) == GAS_MATERIAL_FORM ){
                gas_product   += max(bitcast<f32>(rule.words[base + 1u]), 0.0);
            }
        }
        let capacity_extent = select(
        extent,
