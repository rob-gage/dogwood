...
fn actor_shape_intersects_axis_aligned_cell(
  ...
  if shape.kind == ACTOR_SHAPE_CIRCLE {
    let closest: vec2<f32> =
      max(abs(relative) - vec2<f32>(cell_half_extent), vec2<f32>(0.0));
  ...
  if shape.kind == ACTOR_SHAPE_CAPSULE {
    let segment: f32 = clamp(up, -shape.parameters.y, shape.parameters.y);
    let conservative_radius: f32 =
      shape.parameters.x + cell_half_extent * sqrt(2.0);
    return
      dot(
  ...
  let cell_radius_on_up: f32 =
    cell_half_extent * (abs(shape.up.x) + abs(shape.up.y));
  if
  ...
  let actor_radius_x: f32 =
    shape.parameters.x * abs(shape.tangent.x) + shape.parameters.y * abs(
      shape.up.x);
  let actor_radius_y: f32 =
    shape.parameters.x * abs(shape.tangent.y) + shape.parameters.y * abs(
      shape.up.y);
  return
