...
fn physical_cell_index_from_world_cell(
  ...
  buffered_tile_size: vec2<u32>,
  ring_offset: vec2<u32>,) -> u32 {
  let world_tile: vec2<i32> =
    vec2<i32>(
  ...
  let physical_tile: vec2<u32> =
    physical_tile_from_logical_tile(
      ...
      buffered_tile_size,
      ring_offset,);
  let local_cell: vec2<u32> =
    vec2<u32>(world_cell - world_tile * i32(CELLS_PER_TILE),);
  return
    (physical_tile.y * buffered_tile_size.x + physical_tile.x) * CELL_COUNT_PER_TILE
      + local_cell.y * CELLS_PER_TILE
      + local_cell.x;
}
...
fn world_cell_from_physical_tile_ring_index(
  ...
  return
    (buffered_origin + vec2<i32>(logical_tile)) * i32(CELLS_PER_TILE) + vec2<
      i32
    >(
