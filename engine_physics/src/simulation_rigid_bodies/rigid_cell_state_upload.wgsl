struct Record { slot: u32, integrity: u32, amount: u32, temperature: u32 }
...
fn upload(@builtin(global_invocation_id) id: vec3<u32>) {
    if (id.x >= count || records[id.x].slot >= arrayLength(&integrities)) { return; }
    let r = records[id.x]; integrities[r.slot] = bitcast<f32>(r.integrity); amounts[r.slot] = bitcast<f32>(r.amount); temperatures[r.slot] = bitcast<f32>(r.temperature);
}
