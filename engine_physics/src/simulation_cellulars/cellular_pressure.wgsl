...
@group(0) @binding(0) var<storage, read_write> cellular_material_identifiers: array<
  u32
>;
...
@group(0) @binding(3) var<storage, read_write> cellular_kinematics: array<
  vec4<f32>
>;
@group(0) @binding(4) var<storage, read> cellular_static_properties: array<
  StaticProperties
>;
@group(0) @binding(5) var<storage, read> cellular_dynamic_properties: array<
  vec4<f32>
>;
@group(0) @binding(6) var<storage, read_write> pending_pressure: array<
  vec4<f32>
>;
...
@group(0) @binding(9) var<storage, read_write> retained_pressure: array<
  vec4<f32>
>;
...
@group(0) @binding(14) var<storage, read_write> active_pressure_tiles: array<
  atomic<u32>
>;
@group(0) @binding(15) var<storage, read_write> active_pressure_tile_indices: array<
  u32
>;
@group(0) @binding(16) var<storage, read_write> mechanical_fluid_cells: array<
  MechanicalFluidCell
>;
@group(0) @binding(17) var<storage, read> fluid_pressure_properties: array<
  vec4<f32>
>;
...
>;
@group(0) @binding(38) var<storage, read_write> material_mutation_request_count: array<
  atomic<u32>
>;
@group(0) @binding(22) var<storage, read> rigid_owners: array<u32>;
@group(0) @binding(23) var<storage, read> rigid_material_identifiers: array<
  u32
>;
...
@group(0) @binding(27) var<storage, read_write> rigid_reactions: array<
  RigidReaction
>;
...
@group(0) @binding(30) var<storage, read_write> rigid_predicted_motion: array<
  RigidPredictedMotion
>;
@group(0) @binding(31) var<storage, read_write> rigid_cell_integrities: array<
  f32
>;
@group(0) @binding(32) var<storage, read_write> rigid_damage: array<
  atomic<u32>
>;
@group(0) @binding(33) var<storage, read> rigid_claims: array<u32>;
@group(0) @binding(34) var<storage, read_write> rigid_fractures: array<
  atomic<u32>
>;
@group(0) @binding(36) var<storage, read_write> rigid_fracture_count: atomic<
  u32
>;
@group(1) @binding(0) var<storage, read_write> pressure_indirect_dispatch: array<
  atomic<u32>
>;
@group(1) @binding(1) var<storage, read_write> rigid_damage_dispatch: array<
  atomic<u32>
>;
...
fn propagate_pending_cellular_pressure(
  ...
  for (var channel: u32 = 0u; channel < 4u; channel++) {
    retained_pressure[index][channel] =
      source[channel] - calculate_outgoing_cellular_pressure(
        cell,
        channel,
        source[channel]);
    pressure_b[index][channel] =
      gather_pending_cellular_pressure(cell, channel);
  }
}
...
fn finalize_cellular_pressure(
      ...
      material) == CELLULAR_STATIC_MATERIAL_FORM
  {
    ...
    material_form_from_identifier(material) == CELLULAR_STATIC_MATERIAL_FORM
  {
...
fn propagate_cellular_pressure(logical_index: u32, read_pressure_a: bool) {
  ...
  let current: vec4<f32> =
    select(pressure_b[index], pressure_a[index], read_pressure_a);
  ...
  for (var channel: u32 = 0u; channel < 4u; channel++) {
    gathered[channel] =
      gather_incoming_cellular_pressure(cell, channel, read_pressure_a,);
  }
  if read_pressure_a {
  ...
  for (var side: i32 = -1; side <= 1; side++) {
    let source_cell: vec2<i32> =
      cell
        ...
        - world_cell_side_offset_from_pressure_channel(channel, side);
    let source: u32 =
      cellular_pressure_physical_cell_index_from_world_cell(source_cell);
...
fn cellular_contact_friction_at_physical_cell_index(index: u32) -> f32 {
  if
    index == INVALID_PHYSICAL_CELL_INDEX || external_body_occupancy[index] != 0u
  {
    return 0.0;
  }
...
fn cellular_contact_mass_at_physical_cell_index(index: u32) -> f32 {
  if
    index != INVALID_PHYSICAL_CELL_INDEX
      && material_form_from_identifier(
        cellular_material_identifiers[index]) == CELLULAR_DYNAMIC_MATERIAL_FORM
      && external_body_occupancy[index] == 0u
  {
    return
...
fn find_rigid_static_contact(source: u32) -> RigidStaticContact {
  ...
  let current: vec2<f32> =
    pose.xy + axis_x * local_center.x + axis_y * local_center.y;
  let angle_step: f32 =
    clamp(motion.z * parameters.delta_time, -3.14159265, 3.14159265);
  ...
  let previous_axis_y: vec2<f32> =
    vec2<f32>(-previous_axis_x.y, previous_axis_x.x);
  ...
  let rotational_expansion: f32 =
    min(CELL_SIZE * 2.0, abs(angle_step) * length(local_center));
  let extent: f32 = CELL_RADIUS + rotational_expansion;
  let minimum: vec2<i32> =
    vec2<i32>(floor((min(previous, current) - vec2<f32>(extent)) * 8.0));
  let maximum: vec2<i32> =
    vec2<i32>(floor((max(previous, current) + vec2<f32>(extent)) * 8.0));
  ...
  for (var y: i32 = minimum.y; y <= maximum.y; y++) {
    for (var x: i32 = minimum.x; x <= maximum.x; x++) {
      ...
      let index: u32 =
        cellular_pressure_physical_cell_index_from_world_cell(world_cell);
      if index == INVALID_PHYSICAL_CELL_INDEX {
        continue;
      }
      let material: u32 = cellular_material_identifiers[index];
      ...
      let overlap: vec4<f32> =
        rigid_static_overlap_contact(current, axis_x, axis_y, static_center);
      if overlap.w >= 0.0 {
        let normal: vec2<f32> = overlap.xy;
        let blocking: f32 = max(0.0, -dot(inward_motion, normal));
        if
          blocking > best_blocking || (blocking == best_blocking && overlap.w > best_overlap.penetration)
        {
          ...
          best_overlap =
            RigidStaticContact(
              ...
              current
                ...
                - axis_y * sign(dot(axis_y, normal)) * CELL_HALF,
              overlap.w);
        }
        continue;
      }
      let hit: vec4<f32> =
        rigid_static_swept_contact(previous, current, static_center, extent);
      let time: f32 = hit.z;
...
fn rigid_static_overlap_contact(
  ...
  axis_x: vec2<f32>,
  axis_y: vec2<f32>,
  static_center: vec2<f32>,) -> vec4<f32> {
  ...
  for (var index: u32 = 0u; index < 4u; index++) {
    ...
    let rigid_radius: f32 =
      CELL_HALF * (abs(dot(axis_x, axis)) + abs(dot(axis_y, axis)));
    let static_radius: f32 = CELL_HALF * (abs(axis.x) + abs(axis.y));
    let penetration: f32 =
      rigid_radius + static_radius - abs(dot(difference, axis));
    if penetration < 0.0 {
...
fn accumulate_rigid_sweep_reaction(
  ...
  if
    any(impulse != impulse)
      || any(abs(impulse) > vec2<f32>(1000000.0))
      || abs(torque) > 1000000.0
  {
    atomicStore(&rigid_reactions[body].overflow, 1);
    return;
...
fn saturating_rigid_atomic_add(
  ...
  loop {
    ...
    if accumulator == 0u {
      let result =
        atomicCompareExchangeWeak(&rigid_reactions[body].impulse_x, old, next);
      if result.exchanged {
        return overflowed;
      }
      ...
      let result =
        atomicCompareExchangeWeak(&rigid_reactions[body].impulse_y, old, next);
      if result.exchanged {
        return overflowed;
      }
      old = result.old_value;
...
fn cellular_pressure_world_cell_from_logical_index(logical_index: u32) -> vec2<
  i32
> {
  return
    world_cell_from_logical_tile_major_index(
