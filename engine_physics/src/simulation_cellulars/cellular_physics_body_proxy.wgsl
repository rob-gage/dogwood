...
@group(0) @binding(4) var<storage, read_write> rigid_material_identifiers: array<
  u32
>;
...
@group(0) @binding(11) var<storage, read_write> actor_claims: array<
  atomic<u32>
>;
@group(0) @binding(12) var<storage, read> destroy_requests: array<u32>;
@group(0) @binding(13) var<storage, read_write> destroy_results: array<
  vec2<u32>
>;
...
fn actor_bounds(actor: ActorProxy) -> vec4<i32> {
  let shape =
    actor_shape_from_parameters(
      ...
      actor.shape_kind);
  let extent =
    actor_shape_world_extent(shape) + vec2<f32>(0.5 / CELLS_PER_TILE_FLOAT);
  return
    vec4<i32>(
  ...
  for (var candidate = lane; candidate < count; candidate += 64u) {
    ...
    let index =
      physical_cell_index_from_world_cell(
        ...
        parameters.ring_offset);
    if
      index != INVALID_PHYSICAL_CELL_INDEX && atomicLoad(
...
fn rigid_cell_world_bounds(index: u32) -> vec4<i32> {
  ...
  let local_center: vec2<f32> =
    (vec2<f32>(cell.local) + vec2<f32>(0.5)) / CELLS_PER_TILE_FLOAT;
  let center: vec2<f32> =
    transform.xy + vec2<f32>(
  ...
  let minimum: vec2<i32> =
    vec2<i32>(floor(center * CELLS_PER_TILE_FLOAT - extent));
  let maximum: vec2<i32> =
    vec2<i32>(ceil(center * CELLS_PER_TILE_FLOAT + extent));
...
fn rigid_source_contains_world_cell(
  ...
  let local_center: vec2<f32> =
    (vec2<f32>(cell.local) + vec2<f32>(0.5)) / CELLS_PER_TILE_FLOAT;
  let rigid_center: vec2<f32> =
    transform.xy + axis_x * local_center.x + axis_y * local_center.y;
  ...
  for (var index: u32 = 0u; index < 4u; index++) {
    ...
    let rigid_radius: f32 =
      half * (abs(dot(axis_x, axis)) + abs(dot(axis_y, axis)));
