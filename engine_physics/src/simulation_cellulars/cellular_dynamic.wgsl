...
@group(0) @binding(0) var<storage, read> cellular_material_identifiers: array<
  u32
>;
@group(0) @binding(1) var<storage, read> cellular_appearances: array<u32>;
@group(0) @binding(2) var<storage, read_write> cellular_kinematics: array<
  vec4<f32>
>;
@group(0) @binding(3) var<storage, read_write> material_identifiers_output: array<
  u32
>;
@group(0) @binding(4) var<storage, read_write> appearances_output: array<u32>;
@group(0) @binding(5) var<storage, read_write> destination_claims: array<
  atomic<u32>
>;
...
fn calculate_cellular_dynamic_movement_proposals(
  ...
  var velocity: vec2<f32> =
    cellular_kinematics[source_index].xy + parameters.gravity * parameters.delta_time;
  let maximum_velocity: f32 =
    f32(parameters.maximum_movement_cells) / (f32(
    ...
    if path.direct_blocked && all(destination_cell == source_cell) {
      ...
      if all(destination_cell == source_cell) {
        ...
        if gravity_length > 0.0 {
          let gravity_direction: vec2<f32> =
            parameters.gravity / gravity_length;
  ...
  if source_inside_body && all(destination_cell == source_cell) {
    destination_cell =
      choose_cellular_dynamic_external_body_exit_destination(
        source_cell,
        velocity,);
  }
...
fn resolve_cellular_dynamic_movement_proposals(
  ...
  if claim != INVALID_CELLULAR_DYNAMIC_CLAIM_INDEX {
    let source_index: u32 =
      cellular_dynamic_source_physical_cell_index_from_claim_ticket(
        claim,
        cell);
    material_identifiers_output[index] =
      cellular_material_identifiers[source_index];
      ...
      material_identifier) == CELLULAR_DYNAMIC_MATERIAL_FORM
  {
    ...
    if
      proposal.destination_physical_cell_index != index && atomicLoad(
...
fn choose_cellular_dynamic_slide_destination(
  source_world_cell: vec2<i32>) -> vec2<i32> {
  if all(parameters.gravity == vec2<f32>(0.0)) {
    return source_world_cell;
  }
  let tangent: vec2<f32> =
    vec2<f32>(-parameters.gravity.y, parameters.gravity.x);
  let first_sign: f32 =
    select(
      -1.0,
      1.0,
  ...
  for (var side: u32 = 0u; side < 2u; side++) {
    ...
    let gravity_relative_direction: vec2<f32> =
...
fn trace_cellular_dynamic_displacement(
  ...
  displacement: vec2<i32>,) -> CellularDynamicPathTrace {
...
fn cellular_dynamic_destination_claim_ticket(
  ...
  destination_world_cell: vec2<i32>,) -> u32 {
  return
...
fn hash_cellular_dynamic_claim_priority(
  ...
  tick: u32) -> u32 {
  var value: u32 =
    u32(world_cell.x) * 0x9e3779b9u
      ^ u32(world_cell.y) * 0x85ebca6bu
      ^ tick * 0xc2b2ae35u;
  value ^= value >> 16u;
  value *= 0x7feb352du;
  value ^= value >> 15u;
