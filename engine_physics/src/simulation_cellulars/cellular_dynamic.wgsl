...
fn clear_cellular_dynamic_destination_claims(
...
fn calculate_cellular_dynamic_movement_proposals(
  ...
  // map the logical invocation through the physical tile ring
    ...
    if material_form_from_identifier(
      material_identifier) != CELLULAR_DYNAMIC_MATERIAL_FORM {
        return;
    }

  // integrate arbitrary gravity and enforce the per-tick transport limit
    var velocity: vec2<f32> = cellular_kinematics[source_index].xy + parameters.gravity * parameters.delta_time;
    let maximum_velocity: f32 = f32(parameters.maximum_movement_cells) / (f32(
    ...
    var accumulated: vec2<f32> = cellular_kinematics[source_index].zw + velocity * f32(CELLS_PER_TILE) * parameters.delta_time;
    let accumulated_distance: f32 = length(accumulated);
    if accumulated_distance > f32(parameters.maximum_movement_cells) {
        accumulated   *=
      f32(parameters.maximum_movement_cells) / accumulated_distance;
    }
  ...
  // separate whole-cell displacement from retained subcell residual
    ...
    if any(displacement != vec2<i32>(0)) {
        let path: CellularDynamicPathTrace = trace_cellular_dynamic_displacement(source_cell, displacement,);
  ...
  // publish integrated matter state before atomically competing for its destination
    let destination_index: u32 = physical_cell_index_from_world_cell(
      ...
      parameters.buffered_tile_size,
      parameters.ring_offset,);
    proposals[source_index] = Proposal(destination_index, 0u, 0u, 0u, vec4<f32>(velocity, residual),);
...
fn resolve_cellular_dynamic_movement_proposals(
  ...
  // copy every matter-attached field from an incoming winning source
    if claim != INVALID_CELLULAR_DYNAMIC_CLAIM_INDEX {
        let source_index: u32 = cellular_dynamic_source_physical_cell_index_from_claim_ticket(
        claim,
        cell);
  ...
  // clear accepted sources while preserving losing or stationary dynamic cells
    ...
    if material_form_from_identifier(
      material_identifier) == CELLULAR_DYNAMIC_MATERIAL_FORM {
        ...
        if proposal.destination_physical_cell_index != index && atomicLoad(
        &destination_claims[proposal.destination_physical_cell_index]) == cellular_dynamic_destination_claim_ticket(
        index,
        world_cell_from_physical_tile_ring_index(
...
fn choose_cellular_dynamic_slide_destination(
    source_world_cell: vec2<i32>
) -> vec2<i32> {
    if all(parameters.gravity == vec2<f32>(0.0)) {
    ...
    let first_sign: f32 = select(
      ...
      (hash_cellular_dynamic_claim_priority(
        source_world_cell,
        parameters.tick) & 1u) == 0u,);
    for (var side: u32 = 0u; side < 2u; side++) {
        let tangent_sign: f32 = select(first_sign, -first_sign, side == 1u);
        let gravity_relative_direction: vec2<f32> = parameters.gravity + tangent * tangent_sign;
        ...
        parameters.buffered_tile_size,
        parameters.ring_offset,);
        if any(direction != vec2<i32>(0)) && index != INVALID_PHYSICAL_CELL_INDEX && cellular_material_identifiers[index] == EMPTY_MATERIAL_IDENTIFIER && external_body_occupancy[index] == 0u {
...
fn trace_cellular_dynamic_displacement(
    ...
    for (var step: u32 = 0u; step < parameters.maximum_movement_cells; step++) {
        ...
        parameters.buffered_tile_size,
        parameters.ring_offset,);
        if next_index == INVALID_PHYSICAL_CELL_INDEX || cellular_material_identifiers[next_index] != EMPTY_MATERIAL_IDENTIFIER || external_body_occupancy[next_index] != 0u {
...
fn choose_cellular_dynamic_external_body_exit_destination(
    ...
    let source_index: u32 = physical_cell_index_from_world_cell(
      ...
      parameters.buffered_tile_size,
      parameters.ring_offset,);
    if source_index != INVALID_PHYSICAL_CELL_INDEX && external_body_occupancy[source_index] == 3u && any(abs(velocity) > vec2<f32>(0.0001)) {
        ...
        parameters.buffered_tile_size,
        parameters.ring_offset,);
        if velocity_exit != INVALID_PHYSICAL_CELL_INDEX && cellular_material_identifiers[velocity_exit] == EMPTY_MATERIAL_IDENTIFIER && external_body_occupancy[velocity_exit] == 0u {
            return source_world_cell + along_velocity;
        }
    }
    let first = hash_cellular_dynamic_claim_priority(
      source_world_cell,
      parameters.tick) & 3u;
    for (var offset = 0u; offset < 4u; offset++) {
        ...
        parameters.buffered_tile_size,
        parameters.ring_offset,);
        if index != INVALID_PHYSICAL_CELL_INDEX && cellular_material_identifiers[index] == EMPTY_MATERIAL_IDENTIFIER && external_body_occupancy[index] == 0u {
...
fn cellular_dynamic_destination_claim_ticket(
    ...
    (source_physical_cell_index + cellular_dynamic_destination_claim_offset(
      destination_world_cell)) % parameters.buffered_cell_count;
}
...
fn cellular_dynamic_destination_claim_offset(
    destination_world_cell: vec2<i32>
) -> u32 {
    return
    hash_cellular_dynamic_claim_priority(
      destination_world_cell,
      parameters.tick) % parameters.buffered_cell_count;
}

// Hash stable world coordinates and the simulation tick
fn hash_cellular_dynamic_claim_priority(
    world_cell: vec2<i32>,
    tick: u32
) -> u32 {
    var value: u32 = u32(world_cell.x) * 0x9e3779b9u ^ u32(world_cell.y) * 0x85ebca6bu ^ tick * 0xc2b2ae35u;
    value   ^= value >> 16u;
    value   *= 0x7feb352du;
    value   ^= value >> 15u;
    return value;
