...
fn clear_active_cellular_pressure_tiles(@builtin(global_invocation_id) invocation: vec3<u32>) {
    let tile_count: u32 = cellular_pressure_parameters.buffered_tile_size.x * cellular_pressure_parameters.buffered_tile_size.y;
...
fn mark_active_cellular_pressure_tiles(
    ...
    let logical_tile: vec2<u32> =
        vec2<u32>(
            logical_tile_index % cellular_pressure_parameters.buffered_tile_size.x,
            logical_tile_index / cellular_pressure_parameters.buffered_tile_size.x,
        );
    ...
    let cell_start: u32 =
        (physical_tile.y * cellular_pressure_parameters.buffered_tile_size.x + physical_tile.x) * CELL_COUNT_PER_TILE;
        ...
        for (var offset_x: i32 = -1; offset_x <= 1; offset_x++) {
            ...
            if
                any(active_tile < vec2<i32>(0))
                    || active_tile.x >= i32(cellular_pressure_parameters.buffered_tile_size.x)
                    || active_tile.y >= i32(cellular_pressure_parameters.buffered_tile_size.y)
            {
                continue;
            }
            atomicStore(
                &active_pressure_tiles[u32(active_tile.y) * cellular_pressure_parameters.buffered_tile_size.x + u32(
                    active_tile.x,
                )],
                1u,
            );
...
fn mark_pressure_active_cellular_pressure_tiles(
    ...
    let index: u32 =
        (physical_tile.y * cellular_pressure_parameters.buffered_tile_size.x + physical_tile.x) * CELL_COUNT_PER_TILE + lane;
    if
        any(pending_pressure[index] != vec4<f32>(0.0))
            || external_body_occupancy[index] == 1u
            || external_body_occupancy[index] == 2u
...
fn queue_cellular_radial_impulse(@builtin(global_invocation_id) invocation: vec3<u32>) {
    if invocation.x >= cellular_pressure_parameters.impulse_size.x * cellular_pressure_parameters.impulse_size.y {
        return;
    }
    let cell: vec2<i32> =
        cellular_pressure_parameters.impulse_min + vec2<i32>(
            i32(invocation.x % cellular_pressure_parameters.impulse_size.x),
            i32(invocation.x / cellular_pressure_parameters.impulse_size.x),
        );
    ...
    let delta: vec2<f32> = vec2<f32>(cell) + vec2<f32>(0.5) - cellular_pressure_parameters.impulse_center;
    let distance: f32 = length(delta);
    if distance > cellular_pressure_parameters.impulse_radius {
        return;
    }
    let direction: vec2<f32> = normalize(select(vec2<f32>(1.0, 0.0), delta, distance > 0.0001));
    let falloff: f32 = 1.0 - distance / max(cellular_pressure_parameters.impulse_radius, 0.0001);
    let impulse: vec2<f32> = direction * cellular_pressure_parameters.impulse_strength * falloff;
