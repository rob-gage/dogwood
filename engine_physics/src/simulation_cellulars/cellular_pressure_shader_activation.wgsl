...
fn mark_active_cellular_pressure_tiles(
    ...
    let cell_start: u32 = (physical_tile.y * parameters.buffered_tile_size.x + physical_tile.x) * CELL_COUNT_PER_TILE;
    ...
    let cell: vec2<i32> = world_cell_from_logical_tile_major_index(
      ...
      parameters.buffered_tile_size,);
    var has_source: bool = any(pending_pressure[index] != vec4<f32>(0.0)) || external_body_occupancy[index] == 1u || external_body_occupancy[index] == 2u;
    ...
    for (var channel: u32 = 0u; channel < 4u && !has_source; channel++) {
        ...
        let fluid_interface: bool = (mechanical_fluid_cells[index].mass > 0.0) != (mechanical_fluid_cells[neighbor].mass > 0.0) && (form == CELLULAR_DYNAMIC_MATERIAL_FORM || form == CELLULAR_STATIC_MATERIAL_FORM || neighbor_form == CELLULAR_DYNAMIC_MATERIAL_FORM || neighbor_form == CELLULAR_STATIC_MATERIAL_FORM || gas_cell_is_open(index) || gas_cell_is_open(neighbor));
        let gas_interface: bool = (gas_cell_is_open(index) != gas_cell_is_open(neighbor)) && (form != 0u || neighbor_form != 0u || mechanical_fluid_cells[index].mass > 0.0 || mechanical_fluid_cells[neighbor].mass > 0.0) && (length(gas_velocity[index]) > 0.0001 || length(
          gas_velocity[neighbor]) > 0.0001);
        var granular_impact: bool = false;
        if form == CELLULAR_DYNAMIC_MATERIAL_FORM && (neighbor_form == CELLULAR_DYNAMIC_MATERIAL_FORM || neighbor_form == CELLULAR_STATIC_MATERIAL_FORM) {
            granular_impact = dot(
    ...
    for (var offset_y: i32 = -1; offset_y <= 1; offset_y++) {
        for (var offset_x: i32 = -1; offset_x <= 1; offset_x++) {
            ...
            if any(active_tile < vec2<i32>(0)) || active_tile.x >= i32(parameters.buffered_tile_size.x) || active_tile.y >= i32(parameters.buffered_tile_size.y) {
                continue;
            }
            atomicStore(
...
fn mark_pressure_active_cellular_pressure_tiles(
    ...
    let physical_tile: vec2<u32> = physical_tile_from_logical_tile(
      ...
      parameters.buffered_tile_size,
      parameters.ring_offset,);
    let index: u32 = (physical_tile.y * parameters.buffered_tile_size.x + physical_tile.x) * CELL_COUNT_PER_TILE + lane;
    if any(pending_pressure[index] != vec4<f32>(0.0)) || external_body_occupancy[index] == 1u || external_body_occupancy[index] == 2u {
        atomicStore(&pressure_tile_has_source, 1u);
    }
    workgroupBarrier();
    if lane != 0u || atomicLoad(&pressure_tile_has_source) == 0u {
...
fn compact_active_cellular_pressure_tiles(
    ...
    if logical_tile_index >= tile_count || atomicLoad(
...
fn queue_cellular_radial_impulse(
    ...
    if index == INVALID_PHYSICAL_CELL_INDEX || effective_pressure_material(
