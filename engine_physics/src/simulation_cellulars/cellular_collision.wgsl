...
@group(0) @binding(2) var<uniform> cellular_collision_parameters: CellularCollisionParameters;
...
fn extract_cellular_collision_occupancy(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let tile_count: u32 = cellular_collision_parameters.buffered_tile_size.x * cellular_collision_parameters.buffered_tile_size.y;
    if logical_index >= tile_count {
        return;
    }
    let logical: vec2<u32> =
        vec2<u32>(
            logical_index % cellular_collision_parameters.buffered_tile_size.x,
            logical_index / cellular_collision_parameters.buffered_tile_size.x,
        );
    let physical: vec2<u32> =
        physical_tile_from_logical_tile(
            logical,
            cellular_collision_parameters.buffered_tile_size,
            cellular_collision_parameters.ring_offset,
        );
    let cell_start: u32 =
        (physical.y * cellular_collision_parameters.buffered_tile_size.x + physical.x) * CELL_COUNT_PER_TILE;
