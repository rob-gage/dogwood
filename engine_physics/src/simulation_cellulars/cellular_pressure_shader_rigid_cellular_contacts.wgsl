fn process_rigid_cellular_contact(
    ...
    if dynamic {
        let world_cell: vec2<i32> = world_cell_from_physical_tile_ring_index(
        other,
        parameters.buffered_origin,
        parameters.buffered_tile_size,
        parameters.ring_offset);
        let away: u32 = cellular_pressure_physical_cell_index_from_world_cell(
        ...
        if !constrained {
            constrained = cellular_material_identifiers[away] != EMPTY_MATERIAL_IDENTIFIER || external_body_occupancy[away] != 0u;
        }
    }
    ...
    let shadow: vec4<f32> = vec4<f32>(
      ...
      0.0);
    let rigid_velocity: vec2<f32> = shadow.xy + shadow.z * vec2<f32>(-radius.y, radius.x);
    ...
    if gather {
        ...
        if approach > 0.02 || dynamic && length(other_velocity) > 0.02 || penetration > 0.0001 {
            atomicAdd(&rigid_contact_statistics[body].motion_support[channel], 1u);
        }
        return;
    }
    let normal_arm: f32 = radius.x * normal.y - radius.y * normal.x;
    let rigid_inverse_effective_mass: f32 = mass_record.z + mass_record.w * normal_arm * normal_arm;
    var other_inverse_mass: f32 = select(
      0.0,
      1.0 / max(
        cellular_dynamic_properties[material_index_from_identifier(material)].x,
        0.000001),
      dynamic);
    other_inverse_mass = select(
      other_inverse_mass,
    ...
    let simultaneous_contacts: f32 = f32(
      max(
    ...
    if dynamic {
        let properties: vec4<f32> = cellular_dynamic_properties[material_index_from_identifier(material)];
        ...
        let properties: vec4<f32> = fluid_pressure_properties[material_index_from_identifier(
    ...
    if constrained && !dynamic {
        ...
        if toward_sum > 0.0001 {
          ...
          (1.0 + clamp(restitution, 0.0, 1.0)) * grain_approach / inverse_mass_sum;
            }
        ...
        transfer_normal_impulse = (1.0 + clamp(restitution, 0.0, 1.0)) * max(approach, 0.0) / inverse_mass_sum;
    } else if inverse_mass_sum > 0.0 {
        transfer_normal_impulse = (1.0 + clamp(restitution, 0.0, 1.0)) * max(approach, 0.0) / (simultaneous_contacts * rigid_inverse_effective_mass + other_inverse_mass);
    }
    var penetration_impulse: f32 = 0.0;
    if constrained && !dynamic && penetration > CELL_SIZE * 0.02 && rigid_inverse_effective_mass > 0.000001 {
        let correction_speed: f32 = min(0.25, (penetration - CELL_SIZE * 0.02) * 0.2 / parameters.delta_time);
        penetration_impulse = max(
        0.0,
    ...
    let geometric_contacts: u32 = atomicLoad(&rigid_contact_statistics[body].geometric_support[channel]);
  ...
  // Rapier owns rigid/granular hard contact; only the material-side transfer remains here.
    ...
    let tangent_inverse_mass: f32 = mass_record.z + mass_record.w * tangent_arm * tangent_arm + other_inverse_mass;
    let transfer_tangent_impulse: f32 = select(
      0.0,
      clamp(
    ...
    let rigid_tangent_inverse_mass: f32 = mass_record.z + mass_record.w * tangent_arm * tangent_arm;
    let constraint_tangent_impulse: f32 = clamp(
      (dot(
        relative,
      ...
      max(friction, 0.0) * constraint_normal_impulse);
    let tangent_impulse: f32 = transfer_tangent_impulse + constraint_tangent_impulse;
    let transfer_impulse: vec2<f32> = normal * transfer_normal_impulse + tangent * transfer_tangent_impulse;
    let impulse: vec2<f32> = normal * normal_impulse + tangent * tangent_impulse;
    if other_inverse_mass > 0.0 {
        let after: vec2<f32> = other_velocity + transfer_impulse * other_inverse_mass;
        let transferred_energy: f32 = max(
        0.0,
    ...
    if penetration_impulse > 0.0 {
        ...
        let support_credit: f32 = max(
        0.0,
        dot(shadow.xy, support_reaction) + shadow.z * support_torque) + 0.5 * max(f32(geometric_contacts), simultaneous_contacts) * (mass_record.z * dot(
          support_reaction,
    ...
    let support_torque: f32 = radius.x * support_reaction.y - radius.y * support_reaction.x;
    let support_credit: f32 = 0.5 * f32(geometric_contacts) * (mass_record.z * dot(support_reaction, support_reaction) + mass_record.w * support_torque * support_torque);
...
fn gas_cell_is_open(index: u32) -> bool {
    return
    cellular_material_identifiers[index] == EMPTY_MATERIAL_IDENTIFIER && external_body_occupancy[index] == 0u && fluid_coverage[index] < 0.85;
}

fn gas_cell_inverse_mass(index: u32) -> f32 {
    var density: f32 = 1.0;
    for (var species: u32 = 0u; species < parameters.gas_count; species++) {
        let concentration: f32 = gas_concentrations[species * parameters.buffered_cell_count + index];
...
fn resolve_gas_cellular_face(gas: u32, cellular: u32, normal: vec2<f32>) {
    ...
    let inverse_cellular_mass: f32 = select(
      0.0,
      1.0 / max(
...
fn resolve_gas_fluid_face(gas: u32, fluid: u32, normal: vec2<f32>) {
    ...
    if mass <= 0.0 || cellular_material_identifiers[fluid] != EMPTY_MATERIAL_IDENTIFIER {
        return;
    }
    let inverse_gas_mass: f32 = gas_cell_inverse_mass(gas);
