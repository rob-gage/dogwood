fn process_rigid_cellular_contact(
    ...
    other_inverse_mass =
        select(other_inverse_mass, 1.0 / max(mechanical_fluid_cells[other].mass, 0.000001), fluid);
    other_inverse_mass = select(other_inverse_mass, gas_cell_inverse_mass(other), gas);
    let simultaneous_contacts: f32 =
        f32(max(atomicLoad(&rigid_contact_statistics[body].motion_support[channel]), 1u));
    ...
    if constrained && !dynamic {
        ...
        transfer_normal_impulse =
            (1.0 + clamp(restitution, 0.0, 1.0)) * max(approach, 0.0) / inverse_mass_sum;
            ...
            && rigid_inverse_effective_mass > 0.000001
    {
        ...
        penetration_impulse =
            max(
                0.0,
                correction_speed - max(-approach, 0.0),
    ...
    let tangent_inverse_mass: f32 =
        mass_record.z + mass_record.w * tangent_arm * tangent_arm + other_inverse_mass;
    ...
    let constraint_tangent_impulse: f32 =
        clamp(
            (dot(relative, tangent) - transfer_tangent_impulse * tangent_inverse_mass) / max(
    ...
    if static_cell || dynamic {
        pending_pressure[other] +=
            encode_directional_pressure(normal * normal_impulse * CONTACT_PRESSURE_TRANSFER);
    }
    if dynamic {
        return;
    }
    ...
    if penetration_impulse > 0.0 {
        ...
        let support_credit: f32 =
            max(0.0, dot(shadow.xy, support_reaction) + shadow.z * support_torque) + 0.5
    ...
    if dynamic {
        ...
        atomicAdd(&rigid_contact_statistics[body].padding_2, select(1u, 0x00010001u, moving_grain));
    }
}
...
fn resolve_fluid_cellular_face(fluid: u32, cellular: u32, normal: vec2<f32>) {
    ...
    let properties: vec4<f32> =
        fluid_pressure_properties[material_index_from_identifier(fluid_state.material_identifier)];
    let restitution: f32 =
        clamp(
            min(properties.z, cellular_material_restitution_at_physical_cell_index(cellular)),
            0.0,
            1.0,
        );
    ...
    let tangent: vec2<f32> = vec2<f32>(-normal.y, normal.x);
    let friction: f32 =
        max(0.0, min(properties.y, cellular_contact_friction_at_physical_cell_index(cellular)));
