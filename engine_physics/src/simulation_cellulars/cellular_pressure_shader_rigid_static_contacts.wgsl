...
fn empty_rigid_static_contact() -> RigidStaticContact {
    return RigidStaticContact(0u, 0u, 0u, 0u, 0u, vec2<f32>(0.0), vec2<f32>(0.0), -1.0);
}
...
fn accumulate_rigid_sweep_reaction(body: u32, impulse: vec2<f32>, radius: vec2<f32>) {
    ...
    var overflowed: bool =
        saturating_rigid_atomic_add(body, i32(round(impulse.x * LINEAR_FIXED_SCALE)), 0u);
    overflowed =
...
fn saturating_rigid_atomic_add(body: u32, value: i32, accumulator: u32) -> bool {
    ...
    loop {
        ...
        if accumulator == 0u {
            ...
            let result =
                atomicCompareExchangeWeak(&rigid_reactions[body].angular_impulse, old, next);
            if result.exchanged {
                return overflowed;
            }
            old = result.old_value;
