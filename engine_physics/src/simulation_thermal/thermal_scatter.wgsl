...
@group(0) @binding(13) var<storage, read_write> rigid_temperature_sum: array<
  atomic<u32>
>;
...
fn scatter_cellular_gas(@builtin(global_invocation_id) invocation: vec3<u32>) {
  ...
  let blocked =
    cellular_materials[i] != 0u
      ...
      || occupancy[i] == 3u;
  let available =
    select(1.0 - clamp(fluid_coverage[i], 0.0, 1.0), 0.0, blocked);
...
fn scatter_fluid_particles(@builtin(global_invocation_id) invocation: vec3<u32>) {
  ...
  let p = particles[i];
  let idx =
    cell_index(fluid_particle_world_cell(p.position, CELLS_PER_TILE_FLOAT));
