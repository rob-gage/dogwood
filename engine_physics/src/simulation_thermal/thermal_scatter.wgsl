...
fn physical_index_from_logical(logical: u32) -> u32 {
    let world = world_cell_from_logical_tile_major_index(
      logical,
      parameters.origin,
      parameters.tiles);
    return
...
fn scatter_cellular_gas(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let blocked = cellular_materials[i] != 0u || claim != 0xffffffffu || occupancy[i] == 1u || occupancy[i] == 3u;
...
fn scatter_fluid_particles(@builtin(global_invocation_id) invocation: vec3<u32>) {
    let i = invocation.x;
    if (i >= parameters.particle_capacity || particles[i].is_active == 0u || particles[i].material_identifier == 0u || particles[i].amount <= 0.000001 ){
        return;
    }
    let p = particles[i];
...
fn accumulate_rigid_temperature_sums(
    ...
    if (slot >= parameters.rigid_capacity || claim_counts[slot] == 0u || rigid_amounts[slot] <= 0.000001 ){
        return;
    }
...
fn apply_rigid_temperatures(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    if (c > 0u && rigid_amounts[i] > 0.000001 ){
        rigid_temperatures[i] = max(bitcast<f32>(atomicLoad(&rigid_temperature_sum[i])) / f32(c), 0.0);
    }
}
