fn phase_cells(@builtin(global_invocation_id) invocation: vec3<u32>) {
    if (invocation.x >= parameters.cell_count || atomicLoad(
      &rigid_claims[invocation.x]) != 0xffffffffu ){
        return;
    }
    let w = world_cell_from_physical_tile_ring_index(
      invocation.x,
      parameters.origin,
      parameters.tiles,
      parameters.ring);
...
fn phase_rigid(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    if (slot >= arrayLength(&rigid_amounts) || slot >= arrayLength(
      &rigid_temperatures) ){
        return;
    }
    ...
    if (replacement == EMPTY_MATERIAL_IDENTIFIER || replacement == source || td == 0xffffffffu || td >= arrayLength(&properties) || material_form_from_identifier(replacement) != FLUID_MATERIAL_FORM ){
        return;
    }
    ...
    rigid_phase_candidates[n] = RigidPhaseCandidate(
      slot,
...
fn phase_particles(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let cell = physical_cell_index_from_world_cell(
      world,
      parameters.origin,
      parameters.tiles,
      parameters.ring);
...
fn phase_gases(
    ...
    if (branch == 0u ){
        ...
        if (!(thermal_material_cold_enabled(
        record) && temperature <= thermal_material_cold_threshold(record)) ){
            return;
        }
        replacement = thermal_material_cold_target(record);
    ...
    if (material_form_from_identifier(replacement) == FLUID_MATERIAL_FORM ){
        gas_fluid_candidates[slot] = GasFluidCandidate(replacement, amount, target_temperature, position);
        return;
    }
