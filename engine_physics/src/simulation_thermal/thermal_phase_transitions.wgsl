...
@group(0) @binding(6) var<storage, read> properties: array<
  ThermalMaterialRecord
>;
...
@group(0) @binding(9) var<storage, read_write> request_count: array<
  atomic<u32>
>;
...
@group(0) @binding(12) var<storage, read_write> gas_fluid_candidates: array<
  GasFluidCandidate
>;
...
@group(0) @binding(16) var<storage, read_write> rigid_phase_candidates: array<
  RigidPhaseCandidate
>;
@group(0) @binding(17) var<storage, read_write> rigid_phase_count: array<
  atomic<u32>
>;
@group(0) @binding(18) var<storage, read_write> fluid_free_indices: array<u32>;
@group(0) @binding(19) var<storage, read_write> fluid_free_count: array<
  atomic<u32>
>;
...
fn should_yield(rate: f32, cell: u32, source: u32) -> bool {
  ...
  let h =
    (cell * 1664525u + source * 1013904223u + parameters.tick * 747796405u);
...
fn transition(
  ...
  let target_dense =
    material_dense_index(replacement, thermal.offsets, thermal.counts);
  if
  ...
  let target_cp =
    thermal_material_specific_heat_capacity(properties[target_dense]);
...
fn phase_rigid(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  let required = amount * max(latent, 0.0);
  if
    (latent > 0.0 && sensible < required || !should_yield(rate, slot, source))
  {
    return;
  }
  ...
  rigid_phase_candidates[n] =
    RigidPhaseCandidate(
      ...
      amount,
      max(
        threshold + signed * max(sensible - required, 0.0) / (amount * tcp),
        0.0),
      rigid.body,
      rigid.local.x,
      rigid.local.y,
...
fn phase_gases(
  ...
  let branch = group.z;
  let slot =
    (branch * parameters.gas_count + species) * parameters.cell_count + cell;
