...
@group(0) @binding(10) var<storage, read> thermal_properties: array<
  ThermalMaterialRecord
>;
...
@group(0) @binding(16) var<storage, read_write> rigid_raster_claim_counts: array<
  atomic<u32>
>;
...
fn count_rigid_claims(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  if (slot < arrayLength(&rigid_raster_claim_counts)) {
    atomicAdd(&rigid_raster_claim_counts[slot], 1u);
  }
}
...
fn gather_thermal_interaction(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  if (canonical != EMPTY_MATERIAL_IDENTIFIER) {
    ...
    if
      (dense != 0xffffffffu
        && dense < arrayLength(&thermal_properties)
        && amount > 0.000001)
    {
  ...
  if (claim < arrayLength(&rigid_cells)) {
    ...
    if
      (dense != 0xffffffffu
        && dense < arrayLength(&thermal_properties)
        && rigid.state_slot < arrayLength(&rigid_amounts))
    {
      let amount = max(rigid_amounts[rigid.state_slot], 0.0);
      let share =
        1.0 / f32(
          max(atomicLoad(&rigid_raster_claim_counts[rigid.state_slot]), 1u));
      let record = thermal_properties[dense];
      let c = amount * thermal_material_specific_heat_capacity(record) * share;
      capacity += c;
      energy += c * rigid_temperatures[rigid.state_slot];
      if (amount > 0.000001) {
        conductivity += thermal_material_conductivity(record);
      }
    }
  ...
  for (var species = 0u; species < parameters.gas_count; species++) {
    let amount =
      max(gas_concentrations[species * parameters.cell_count + index], 0.0);
    gas_amount += amount;
    let dense =
      material_dense_index(
        species + 1u,
        material_parameters.offsets,
        material_parameters.counts);
    if (dense != 0xffffffffu && dense < arrayLength(&thermal_properties)) {
      let record = thermal_properties[dense];
      gas_capacity += amount * thermal_material_specific_heat_capacity(record);
      gas_conductivity += amount * thermal_material_conductivity(record);
    }
  }
  let available =
    select(
  ...
  energy +=
    (gas_capacity + air * parameters.empty_capacity) * gas_temperatures[index];
  conductivity += gas_conductivity + air * parameters.empty_k;
  let equilibrium =
    select(parameters.ambient, energy / capacity, capacity > 0.000001);
  interaction[index] =
    vec4<f32>(capacity, energy, conductivity, max(equilibrium, 0.0));
}
