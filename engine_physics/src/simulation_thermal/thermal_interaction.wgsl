...
fn count_rigid_claims(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let cell = world_cell_from_logical_tile_major_index(
      invocation.x,
...
fn gather_thermal_interaction(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let available = select(
      ...
      0.0,
      canonical != EMPTY_MATERIAL_IDENTIFIER || claim < arrayLength(&rigid_cells) || external_occupancy[index] == 1u || external_occupancy[index] == 3u);
