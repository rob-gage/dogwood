...
fn calculate_gas_vorticity_confinement(
    ...
    return
    parameters.vorticity_confinement * vec2<f32>(normal.y, -normal.x) * curl[physical_cell_index];
}
...
fn calculate_gas_jacobi_pressure(
    ...
    (gas_pressure_at_world_cell(
      ...
      center,
      read_pressure_a) + gas_pressure_at_world_cell(
        world_cell + vec2<i32>(1, 0),
        center,
        read_pressure_a) + gas_pressure_at_world_cell(
        ...
        world_cell + vec2<i32>(0, 1),
        center,
        read_pressure_a) - divergence[physical_cell_index]) * 0.25;
}
...
fn gas_concentration_flux_across_positive_x_face(
    ...
    let face_velocity: f32 = 0.5 * (gas_velocity_at_world_cell(left).x + gas_velocity_at_world_cell(
      right).x);
...
fn gas_concentration_flux_across_positive_y_face(
    ...
    let face_velocity: f32 = 0.5 * (gas_velocity_at_world_cell(bottom).y + gas_velocity_at_world_cell(
      top).y);
...
fn world_cell_is_gas_obstacle(cell: vec2<i32>) -> bool {
    let index: u32 = gas_physical_cell_index_from_world_cell(cell);
    return
    index == INVALID_PHYSICAL_CELL_INDEX || cellular_material_identifiers[index] != EMPTY_MATERIAL_IDENTIFIER || external_body_occupancy[index] != 0u || fluid_coverage[index] >= parameters.fluid_obstacle_coverage;
}
