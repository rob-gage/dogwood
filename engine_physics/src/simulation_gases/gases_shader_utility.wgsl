...
fn calculate_gas_jacobi_pressure(
    ...
    let center: f32 =
        select(pressure_b[physical_cell_index], pressure_a[physical_cell_index], read_pressure_a);
    return
...
fn clamp_projected_gas_velocity_against_obstacle_faces(
    ...
    if world_cell_is_gas_obstacle(world_cell + vec2<i32>(-1, 0)) && clamped_velocity.x < 0.0 {
        clamped_velocity.x = 0.0;
    }
    if world_cell_is_gas_obstacle(world_cell + vec2<i32>(1, 0)) && clamped_velocity.x > 0.0 {
        clamped_velocity.x = 0.0;
    }
    if world_cell_is_gas_obstacle(world_cell + vec2<i32>(0, -1)) && clamped_velocity.y < 0.0 {
...
fn gas_concentration_flux_across_positive_x_face(species: u32, left: vec2<i32>) -> f32 {
    ...
    let face_velocity: f32 =
        0.5 * (gas_velocity_at_world_cell(left).x + gas_velocity_at_world_cell(right).x);
...
fn gas_concentration_flux_across_positive_y_face(species: u32, bottom: vec2<i32>) -> f32 {
    ...
    let face_velocity: f32 =
        0.5 * (gas_velocity_at_world_cell(bottom).y + gas_velocity_at_world_cell(top).y);
...
fn gas_pressure_at_world_cell(cell: vec2<i32>, boundary: f32, read_pressure_a: bool) -> f32 {
    if world_cell_is_gas_obstacle(cell) {
        return boundary;
    }
