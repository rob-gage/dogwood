...
fn advect_gas_velocity(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    if logical_index >= gas_simulation_parameters.buffered_cell_count {
        return;
    }
    ...
    if world_cell_is_gas_obstacle(cell) {
        velocity_scratch[gas_physical_cell_index] = vec2<f32>(0.0);
        return;
    }
    let center: vec2<f32> = vec2<f32>(cell) + vec2<f32>(0.5);
    let backtraced: vec2<f32> = center - velocity[gas_physical_cell_index] * gas_simulation_parameters.delta_time;
    velocity_scratch[gas_physical_cell_index] = sample_gas_velocity_bilinear_at_cell_position(backtraced);
}
...
fn calculate_gas_curl(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let gas_physical_cell_index: u32 = gas_physical_cell_index_from_world_cell(cell);
    if world_cell_is_gas_obstacle(cell) {
        curl[gas_physical_cell_index] = 0.0;
        return;
    }
    ...
    curl[gas_physical_cell_index] = 0.5 * ((right.y - left.y) - (top.x - bottom.x));
}
...
fn apply_gas_forces(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let buoyancy: vec2<f32> =
        gas_simulation_parameters.gravity
            * CELLS_PER_TILE_FLOAT
            * gas_density_offset_from_ambient(gas_physical_cell_index)
            * gas_simulation_parameters.buoyancy_coefficient;
    let confinement: vec2<f32> = calculate_gas_vorticity_confinement(cell, gas_physical_cell_index);
    var next_velocity: vec2<f32> =
        velocity_scratch[gas_physical_cell_index] + (buoyancy + confinement) * gas_simulation_parameters.delta_time;
    let speed: f32 = length(next_velocity);
    if speed > gas_simulation_parameters.maximum_speed {
        next_velocity *= gas_simulation_parameters.maximum_speed / speed;
    }
    velocity[gas_physical_cell_index] = next_velocity;
}
...
fn calculate_gas_divergence(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let gas_physical_cell_index: u32 = gas_physical_cell_index_from_world_cell(cell);
    if world_cell_is_gas_obstacle(cell) {
        divergence[gas_physical_cell_index] = 0.0;
        return;
    }
    divergence[gas_physical_cell_index] =
...
fn clear_gas_pressure(@builtin(global_invocation_id) invocation: vec3<u32>) {
    if invocation.x >= gas_simulation_parameters.buffered_cell_count {
        return;
    }
...
fn solve_gas_pressure_a(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let gas_physical_cell_index: u32 = gas_physical_cell_index_from_world_cell(cell);
    if world_cell_is_gas_obstacle(cell) {
        pressure_a[gas_physical_cell_index] = 0.0;
        return;
    }
    pressure_a[gas_physical_cell_index] = calculate_gas_jacobi_pressure(cell, gas_physical_cell_index, false);
}
...
fn solve_gas_pressure_b(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let gas_physical_cell_index: u32 = gas_physical_cell_index_from_world_cell(cell);
    if world_cell_is_gas_obstacle(cell) {
        pressure_b[gas_physical_cell_index] = 0.0;
        return;
    }
    pressure_b[gas_physical_cell_index] = calculate_gas_jacobi_pressure(cell, gas_physical_cell_index, true);
}
...
fn project_gas_velocity(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let center_pressure: f32 = pressure_b[gas_physical_cell_index];
    var projected: vec2<f32> =
        velocity[gas_physical_cell_index] - 0.5 * vec2<f32>(
    ...
    velocity[gas_physical_cell_index] = clamp_projected_gas_velocity_against_obstacle_faces(cell, projected);
}
...
fn advect_gas_concentrations(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let field_count: u32 = gas_simulation_parameters.buffered_cell_count * gas_simulation_parameters.gas_count;
    if field_index >= field_count {
        return;
    }
    let species: u32 = field_index / gas_simulation_parameters.buffered_cell_count;
    let logical_index: u32 = field_index % gas_simulation_parameters.buffered_cell_count;
    let cell: vec2<i32> = world_cell_from_gas_logical_index(logical_index);
    let gas_physical_cell_index: u32 = gas_physical_cell_index_from_world_cell(cell);
    let output_index: u32 =
        gas_concentration_storage_index_from_species_and_physical_cell(species, gas_physical_cell_index);
    if world_cell_is_gas_obstacle(cell) {
        let remaining: f32 = exp(-max(gas_properties[species * 2u].w, 0.0) * gas_simulation_parameters.delta_time);
    ...
    let transported: f32 =
        current - gas_simulation_parameters.delta_time * (gas_concentration_flux_across_positive_x_face(
            species,
            cell,
        )
    ...
    let mixing: f32 =
        min(
            (max(
                properties.y,
                0.0,
            ) + (1.0 - compressibility) * INCOMPRESSIBILITY_MIXING) * gas_simulation_parameters.delta_time,
            0.24,
        );
    let mixed: f32 =
        transported
    ...
    let remaining: f32 = exp(-max(properties.w, 0.0) * gas_simulation_parameters.delta_time);
