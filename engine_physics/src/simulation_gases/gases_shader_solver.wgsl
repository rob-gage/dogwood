...
fn apply_gas_forces(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let buoyancy: vec2<f32> = parameters.gravity * CELLS_PER_TILE_FLOAT * gas_density_offset_from_ambient(index) * parameters.buoyancy_coefficient;
    let confinement: vec2<f32> = calculate_gas_vorticity_confinement(cell, index);
    var next_velocity: vec2<f32> = velocity_scratch[index] + (buoyancy + confinement) * parameters.delta_time;
...
fn calculate_gas_divergence(
    ...
    divergence[index] = 0.5 * (gas_velocity_at_world_cell(cell + vec2<i32>(1, 0)).x - gas_velocity_at_world_cell(cell + vec2<i32>(-1, 0)).x + gas_velocity_at_world_cell(cell + vec2<i32>(0, 1)).y - gas_velocity_at_world_cell(cell + vec2<i32>(0, -1)).y);
}
...
fn advect_gas_concentrations(
    ...
    let cell: vec2<i32> = world_cell_from_gas_logical_index(logical_index);
    let index: u32 = gas_physical_cell_index_from_world_cell(cell);
    let output_index: u32 = gas_concentration_storage_index_from_species_and_physical_cell(
      species,
      index);
    if world_cell_is_gas_obstacle(cell) {
        let remaining: f32 = exp(-max(gas_properties[species * 2u].w, 0.0) * parameters.delta_time);
        concentration_scratch[output_index] = select(
    ...
    let transported: f32 = current - parameters.delta_time * (gas_concentration_flux_across_positive_x_face(
      species,
      cell) - gas_concentration_flux_across_positive_x_face(
        species,
        cell + vec2<i32>(-1, 0)) + gas_concentration_flux_across_positive_y_face(species, cell) - gas_concentration_flux_across_positive_y_face(
        species,
        cell + vec2<i32>(0, -1)));
    ...
    let mixing: f32 = min(
      ...
      0.24,);
    let mixed: f32 = transported + mixing * (gas_concentration_at_world_cell(
        species,
        cell + vec2<i32>(-1, 0),
        current) + gas_concentration_at_world_cell(
          species,
          cell + vec2<i32>(1, 0),
          current) + gas_concentration_at_world_cell(
          species,
          ...
          species,
          cell + vec2<i32>(0, 1),
          current) - 4.0 * current) + gas_displaced_into_open_cell(species, cell);
