...
struct Parameters { ambient: f32, empty_k: f32, empty_capacity: f32, cell_count: f32, gas_count: f32, padding: vec3<u32>, buffered_origin: vec2<i32>, buffered_tiles: vec2<u32>, ring_offset: vec2<u32> }
...
fn gather_thermal_interaction(@builtin(global_invocation_id) id: vec3<u32>) {
    if (id.x >= u32(parameters.cell_count)) { return; }
    ...
    if (canonical != EMPTY_MATERIAL_IDENTIFIER) {
        ...
        if (dense != 0xffffffffu && dense < arrayLength(&thermal_properties)) {
            ...
            capacity += c; energy += c * cellular_temperatures[index]; conductivity += amount * thermal_material_conductivity(record);
        }
    }
    ...
    if (claim != 0xffffffffu) {
        ...
        if (dense != 0xffffffffu && dense < arrayLength(&thermal_properties) && rigid.state_slot < arrayLength(&rigid_amounts)) {
            let record = thermal_properties[dense]; let c = max(rigid_amounts[rigid.state_slot], 0.0) * thermal_material_specific_heat_capacity(record);
            capacity += c; energy += c * rigid_temperatures[rigid.state_slot]; conductivity += max(rigid_amounts[rigid.state_slot], 0.0) * thermal_material_conductivity(record);
        }
    }
    ...
    if (canonical == EMPTY_MATERIAL_IDENTIFIER && claim == 0xffffffffu) {
        for (var species=0u; species < u32(parameters.gas_count); species++) {
            let amount = max(gas_concentrations[species * u32(parameters.cell_count) + index], 0.0); gas_amount += amount;
        ...
        let air = clamp(1.0 - clamp(fluid_coverage[index], 0.0, 1.0) - gas_amount, 0.0, 1.0);
