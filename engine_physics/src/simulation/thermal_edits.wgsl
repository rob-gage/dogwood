...
struct Parameters { ring_origin: vec2<i32>, ring_tiles: vec2<u32>, ring_offset: vec2<u32>, capacity: u32, request_count: u32 }
...
@group(0) @binding(10) var<storage, read_write> rigid_flags: array<u32>;
@group(0) @binding(11) var<storage, read_write> rigid_deltas: array<f32>;
...
fn apply_thermal_requests(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if (claim != 0xffffffffu) { let slot = rigid_cells[claim].state_slot; if (slot < arrayLength(&rigid_temperatures)) { rigid_deltas[slot] = delta; rigid_flags[slot] = 1u; } }
}
...
fn apply_thermal_rigid(@builtin(global_invocation_id) id: vec3<u32>) {
    if (id.x >= arrayLength(&rigid_temperatures) || rigid_flags[id.x] == 0u) { return; }
    rigid_temperatures[id.x] = max(0.0, rigid_temperatures[id.x] + rigid_deltas[id.x]);
}

@compute @workgroup_size(64)
fn clear_thermal_edits(@builtin(global_invocation_id) id: vec3<u32>) {
    if (id.x < parameters.capacity) { deltas[id.x] = 0.0; }
    if (id.x < arrayLength(&rigid_flags)) { rigid_flags[id.x] = 0u; rigid_deltas[id.x] = 0.0; }
}
