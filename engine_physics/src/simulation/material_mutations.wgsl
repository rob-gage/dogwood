...
@compute @workgroup_size(64) fn resolve_material_mutations(@builtin(global_invocation_id) invocation: vec3<u32>) {
 let i=invocation.x; if (i >= atomicLoad(&request_count[0]) || i >= arrayLength(&requests)) { return; }
 let request=requests[i]; if (request.cell >= parameters.buffered_cell_count) { return; }
 if (request.kind == 1u) { resolve_particle(request); return; }
 if (request.kind == 2u) { resolve_gas(request); return; }
 if (request.kind != 0u || cellular_material_identifiers[request.cell] != request.expected_source) { return; }
 let form=material_form_from_identifier(request.replacement); let source_amount=cellular_amounts[request.cell]; let source_temperature=cellular_temperatures[request.cell]; let result_amount=select(source_amount,bitcast<f32>(request.amount),request.amount!=0u); let result_temperature=select(source_temperature,bitcast<f32>(request.temperature),request.temperature!=0u); cellular_kinematics[request.cell]=vec4<f32>(0.0);
 if (request.replacement == EMPTY_MATERIAL_IDENTIFIER) { cellular_material_identifiers[request.cell]=0u; cellular_appearances[request.cell]=0u; cellular_integrities[request.cell]=0.0; cellular_amounts[request.cell]=0.0; cellular_temperatures[request.cell]=0.0; fluid_edits[request.cell]=FLUID_EDIT_ERASE; atomicStore(&fluid_edits_pending[0],1u); return; }
 if (form == CELLULAR_STATIC_MATERIAL_FORM) { let index=material_index_from_identifier(request.replacement); if(index >= arrayLength(&static_defaults)){return;} cellular_material_identifiers[request.cell]=request.replacement; cellular_amounts[request.cell]=result_amount;cellular_temperatures[request.cell]=result_temperature; cellular_integrities[request.cell]=bitcast<f32>(static_defaults[index]); return; }
 if (form == CELLULAR_DYNAMIC_MATERIAL_FORM) { cellular_material_identifiers[request.cell]=request.replacement;cellular_amounts[request.cell]=result_amount;cellular_temperatures[request.cell]=result_temperature; cellular_integrities[request.cell]=0.0; return; }
 cellular_material_identifiers[request.cell]=0u; cellular_appearances[request.cell]=0u; cellular_integrities[request.cell]=0.0;
 if (form == FLUID_MATERIAL_FORM) { fluid_edits[request.cell]=request.replacement; fluid_edit_amounts[request.cell]=result_amount; fluid_edit_temperatures[request.cell]=result_temperature; cellular_amounts[request.cell]=0.0;cellular_temperatures[request.cell]=0.0; atomicStore(&fluid_edits_pending[0],1u); return; }
 let species=material_index_from_identifier(request.replacement); if(species >= parameters.gas_count){return;} gas_temperatures[request.cell]=result_temperature; gas_concentrations[species*parameters.buffered_cell_count+request.cell]+=result_amount; cellular_amounts[request.cell]=0.0;cellular_temperatures[request.cell]=0.0; }

fn release_particle(index:u32) { particles[index].material_identifier=EMPTY_MATERIAL_IDENTIFIER; particles[index].is_active=0u; let n=atomicAdd(&fluid_free_count[0],1u); if(n<arrayLength(&fluid_free_indices)){fluid_free_indices[n]=index;} }
fn claim_particle() -> u32 { var n=atomicLoad(&fluid_free_count[0]); loop { if(n==0u){return 0xffffffffu;} let r=atomicCompareExchangeWeak(&fluid_free_count[0],n,n-1u); if(r.exchanged){return fluid_free_indices[n-1u];} n=r.old_value; } return 0xffffffffu; }
fn spawn_fluid(request:Request) { let index=claim_particle(); if(index==0xffffffffu){return;} particles[index]=Particle(request.replacement,1u,vec2<f32>(bitcast<f32>(request.world_x),bitcast<f32>(request.world_y)),vec2<f32>(0.0),vec2<f32>(0.0),bitcast<f32>(request.amount),bitcast<f32>(request.temperature)); }
...
fn resolve_gas(request:Request) { let source=material_index_from_identifier(request.expected_source);if(source>=parameters.gas_count){return;}let at=source*parameters.buffered_cell_count+request.cell;let amount=gas_concentrations[at];if(!(amount>0.000001)){return;}let target_species=material_form_from_identifier(request.replacement);if(target_species==0u){let species=material_index_from_identifier(request.replacement);if(species>=parameters.gas_count){return;}gas_concentrations[at]=0.0;gas_concentrations[species*parameters.buffered_cell_count+request.cell]+=amount;gas_temperatures[request.cell]=bitcast<f32>(request.temperature);return;}if(target_species==FLUID_MATERIAL_FORM){gas_concentrations[at]=0.0;spawn_fluid(request);return;}if(place_cell(request)){gas_concentrations[at]=0.0;} }
