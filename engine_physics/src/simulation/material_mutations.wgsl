...
@compute @workgroup_size(64) fn resolve_material_mutations(@builtin(global_invocation_id) invocation: vec3<u32>) {
 ...
 if (request.replacement == EMPTY_MATERIAL_IDENTIFIER) { cellular_material_identifiers[request.cell]=0u; cellular_appearances[request.cell]=0u; cellular_integrities[request.cell]=0.0; fluid_edits[request.cell]=0u; for(var s=0u;s<parameters.gas_count;s++){gas_concentrations[s*parameters.buffered_cell_count+request.cell]=0.0;} return; }
 if (form == CELLULAR_STATIC_MATERIAL_FORM) { let index=material_index_from_identifier(request.replacement); if(index >= arrayLength(&static_defaults)){return;} cellular_material_identifiers[request.cell]=request.replacement; cellular_integrities[request.cell]=bitcast<f32>(static_defaults[index]); fluid_edits[request.cell]=0u; for(var s=0u;s<parameters.gas_count;s++){gas_concentrations[s*parameters.buffered_cell_count+request.cell]=0.0;} return; }
 if (form == CELLULAR_DYNAMIC_MATERIAL_FORM) { cellular_material_identifiers[request.cell]=request.replacement; cellular_integrities[request.cell]=0.0; fluid_edits[request.cell]=0u; for(var s=0u;s<parameters.gas_count;s++){gas_concentrations[s*parameters.buffered_cell_count+request.cell]=0.0;} return; }
 cellular_material_identifiers[request.cell]=0u; cellular_appearances[request.cell]=0u; cellular_integrities[request.cell]=0.0;
 if (form == FLUID_MATERIAL_FORM) { fluid_edits[request.cell]=request.replacement; for(var s=0u;s<parameters.gas_count;s++){gas_concentrations[s*parameters.buffered_cell_count+request.cell]=0.0;} return; }
 let species=material_index_from_identifier(request.replacement); if(species >= parameters.gas_count){return;} fluid_edits[request.cell]=0u; for(var s=0u;s<parameters.gas_count;s++){gas_concentrations[s*parameters.buffered_cell_count+request.cell]=select(0.0,1.0,s==species);} }
