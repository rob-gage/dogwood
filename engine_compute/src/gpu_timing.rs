...
impl GpuTiming {
    ...
    pub(crate) fn new(
        ...
        #[cfg(debug_assertions)]
        {
            ...
            Self {
                ...
                state: Mutex::new(GpuTimingState::new(readbacks)),
            }
        }
        ...
        if state.query_count + 2 > Self::QUERY_CAPACITY {
            ...
            if !state.capacity_exhausted {
                ...
                tracing::warn!(
                    ...
                    "GPU timing query capacity exhausted"
                );
            }
            return None;
        }
    ...
    fn collect_readback(&self, index: usize) {
        let (sample, records, timestamps, failed) = {
            let mut state = self.state.lock().unwrap_or_else(|error| error.into_inner());
            let readback: &mut GpuTimingReadback = &mut state.readbacks[index];
            let failed: bool = readback.status.load(Ordering::Acquire) == GpuTimingReadback::FAILED;
        ...
        if failed {
            tracing::warn!(target: "dogwood_gpu", sample, "GPU timing readback failed");
            return;
        }
    ...
    #[cfg(debug_assertions)]
    fn report(&self, sample: u64, records: Vec<GpuTimingRecord>, timestamps: &[u64]) {
        ...
        for (index, record) in records.into_iter().enumerate() {
            let Some(ticks) = timestamps[index * 2 + 1].checked_sub(timestamps[index * 2]) else {
                tracing::warn!(
                    ...
                    sample,
                    pass = %record.label,
                    "discarded wrapped GPU timestamp"
                );
                continue;
            };
